...
<h4>문형기</h4> 배운 내용을 전부 제대로 활용하지 못했다는 점이 가장 크게 아쉬웠다. 이는 리뷰가 많이 부족했던 이유가 가장 크다고 생각한다. 또한 프로젝트가 완벽히 마무리되지 못한 점에 대해서 크게 반성하게 된다. 실력이 부족했던 부분과 시간이 갈수록 의지가 빈약해진게 큰 원인이라고 생각한다. 이를 터닝포인트로 삼아 꾸준히 성장하도록 해야겠다.</br></br>
